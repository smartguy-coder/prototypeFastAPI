...
class AuthHandler:
    ...
    async def generate_token_pair(self, user: User) -> LoginResponse:
        ...
        access_token = await self.generate_token(
            access_token_payload, access_token_expires
        )
        ...
        refresh_token = await self.generate_token(
            refresh_token_payload, refresh_token_expires
        )
    ...
    async def get_refresh_token(
        ...
        if not stored_refresh:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Token was used already"
            )
